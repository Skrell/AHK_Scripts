; https://www.autohotkey.com/boards/viewtopic.php?t=33129#p154163
...
oWin := JEE_ExpWinGetObj(hWnd)
JEE_ExpGetInterfaces(oWin, isp, isb, isv, ifv2, icm)
vListAbbrev := "nam,mod,typ,siz"
vList := JEE_ExpColAbbrevToName(vListAbbrev, ",")
; tooltip, % vList
StringSplit, MyArray, vList, `,
MsgBox, The number of items is: %MyArray0%

; MsgBox, % StrReplace(vList, ",", "`n")
; JEE_ICMSetColumns(icm, vList, ",")
colArray := StrSplit(vList, ",")  ; Splits by comma

; Method 1: Display each element in a separate message box
; For index, element in colArray {
    ; MsgBox, % "Element " . index . ": " . element
; }

For index, element in colArray {
	; vWidth := JEE_ICMGetColumnWidth(icm, "System.ItemNameDisplay", vWidthIdeal) ;not working
	vWidth := JEE_ICMGetColumnWidth(icm, element, vWidthIdeal) ;not working
	MsgBox, % element " " vWidth " " vWidthIdeal
}
